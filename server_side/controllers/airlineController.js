    ...
    try {
        ...
        if (existingAirline) {
            if (existingAirline.name.toLowerCase() === name.toLowerCase()) {
                return res.status(409).json({ success: false, message: 'Airline name already exists' });
            }
...
module.exports.getAirlineDetails = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(airlineId)) {
        return res.status(400).json({ success: false, message: 'Invalid airline ID format' });
    }
...
module.exports.updateAirline = async (req, res) => {
    ...
    // Validate ObjectId
    if (!mongoose.Types.ObjectId.isValid(airlineId)) {
        return res.status(400).json({ success: false, message: 'Invalid airline ID' });
    }
