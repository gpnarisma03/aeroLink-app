...
module.exports.addFlight = async (req, res) => {
  ...
  // Basic validation
  if (!airline || !fromLocation || !toLocation || !departureDate || !arrivalDate || seatsAvailable == null) {
    return res.status(400).json({ success: false, message: 'All fields are required' });
  }

  if (fromLocation.toLowerCase() === toLocation.toLowerCase()) {
    return res.status(400).json({ success: false, message: 'From and To locations must be different' });
  }
  ...
  if (seatsAvailable < 0) {
    return res.status(400).json({ success: false, message: 'Seats must be 0 or more' });
  }
