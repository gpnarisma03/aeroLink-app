...
module.exports.registerUser = async (req, res) => {
    ...
    const { firstName, lastName, email, mobileNo, password, dateOfBirth, role } = reqBody;

    // Basic field validation
    if (!firstName || !lastName || !email || !mobileNo || !password || !dateOfBirth) {
        return res.status(400).json({ success: false, message: 'All fields are required' });
    }

    // Mobile number: Must be 11 digits
    if (!/^\d{11}$/.test(mobileNo)) {
        return res.status(400).json({ success: false, message: 'Mobile number must be exactly 11 digits' });
    }
...
module.exports.loginUser = async (req, res) => {
    try {
        ...
        if (!email || !emailRegex.test(email)) {
            return res.status(400).json({ success: false, message: "Invalid or missing email format" });
        }
...
module.exports.userDetails = (req, res) => {
    ...
    User.findById(req.user.id).lean()
        .then(user => {
            ...
            const { _id, firstName, lastName, email, mobileNo, dateOfBirth, bookingHistory, role} = user;
